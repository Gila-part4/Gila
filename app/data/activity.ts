...
export const getMyActivities = cache(
  async ({
    cursor,
    take = 10,
  }: {
...
export const getActivityById = cache(async (id: string): Promise<ActivityWithRequest> => {
  const userId = await getCurrentUserId();
  try {
...
export const getAvailableReviewActivities = cache(
  async ({
    cursor,
    take = 10,
  }: {
...
export const getActivitiesByLocation = cache(
  async ({
    location,
    secondeLocation,
    size = 5,
